...
export default function App() {
  ...
  const triggerSOS = async () => {
    ...
    Vibration.vibrate([100, 200, 100, 200, 1000]);
  ...
  // ─── Fake Call ────────────────────────────────────────────────────────────
  const fakeCallPattern = [0, 1000, 2000, 1000, 2000]; // wait, vibrate, wait, vibrate
  
  const triggerFakeCall = () => {
    setFakeCallActive(true);
    Vibration.vibrate(fakeCallPattern, true); // loop true
...
function FakeCallScreen({ onDismiss }) {
  ...
  // Simple ringtone sound helper (using vibration primary logic)
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <View style={fakeStyles.buttons}>
        ...
        <TouchableOpacity style={[fakeStyles.callBtn, fakeStyles.answer]} onPress={onDismiss /* in real app: navigate to talking state */}>
          <Text style={fakeStyles.callBtnText}>✓</Text>
